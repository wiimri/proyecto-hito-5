# Mercado Vecino - Hito 5 Presentación del proyecto

Marketplace desarrollado para el Hito 5 de Desafio Latam. Esta version esta organizada para desplegar el backend y el cliente en Render, usando Neon como PostgreSQL online, con la estructura de carpetas Backend y Cliente.
...
Cliente (`Cliente/.env` en local o Environment Variables en Render Static Site):
...
| Requisito | Evidencia |
|---|---|
| Estructura de carpetas | Carpetas Backend y Cliente separadas y funcionales. |
| Deploy aplicacion cliente | Static Site de Render desde `Cliente/`. |
| Deploy aplicacion backend | Web Service de Render desde `Backend/`. |
| Deploy base de datos | Neon PostgreSQL con `schema.sql`. |
